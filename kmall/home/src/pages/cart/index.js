...
var page = {
	...
	bindEvent:function(){
		var _this = this;
		//1.单条商品选中/取消
		_this.$elem.on('click','.select-one',function(){
			var $this = $(this);
		...
		//2.全选/全不选
		_this.$elem.on('click','.select-all',function(){
			var $this = $(this);
			//全选中
			if($this.is(":checked")){
		...
		//3.删除一条
		_this.$elem.on('click','.delete-one',function(){
		...
		//4.删除选中
		_this.$elem.on('click','.delete-selected',function(){
			if(_util.confirm('您确定要删除选中购物车记录吗?')){
		...
		//6.去结算
		_this.$elem.on('click','.btn-submit',function(){
