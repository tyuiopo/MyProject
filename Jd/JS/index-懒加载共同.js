;(function($){
	...
	//只加载一次html函数
		...
		*/
		function LazyLoad(options){
			...
			$elem.on(eventName,loadFn = function(ev,index,elem){
				...
				if(item[index] != 'loaded'){
					//具体的加载分离，在具体的调用时监听事件eventPerfix+'-load'来执行
		...
		var $todayCarousel = $('.today .carousel-wrap')
			$todayCarousel.on('carousel-load',function(ev,index,elem,success){
				var $imgs = $(elem).find('.carousel-img');
				$imgs.each(function(){
					var $img = $(this);
					var imgUrl = $img.data('src');
		...
		// carouselImgLazyLoad($todayCarousel)
		LazyLoad({
			totalItemNum:$todayCarousel.find('.carousel-img').length,
			$elem:$todayCarousel,
			eventName:'carousel-show',
			eventPerfix:'carousel'
		})
	...
	function buildFloorBodyHtml(oneFloorData){
		...
		for(var i = 0;i<oneFloorData.items.length;i++){
			...
			for(var j = 0;j<oneFloorData.items[i].length;j++){
				...
				html += '				<a href="#">';
				html += '					<img class="floor-img" src="img/load.jpg" data-src="img/floor/'+oneFloorData.num+'/'+(i+1)+'/'+(j+1)+'.png" alt="">';
				html += '				</a>';
				html += '			</p>';
	...
	/*  function floorHtmlLazyLoad(){
		...
		$doc.on('floor-load',function(ev,index,elem){
			...
			//加载HTML
			//1.生成HTML
			getDataOnce($doc,'data/floor-data/floor-Data.json',function(data){
				console.log(data)
				var $elem = $(elem);
	...
	LazyLoad({
		totalItemNum:$floor.find('.floor-img').length,
		$elem:$doc,
